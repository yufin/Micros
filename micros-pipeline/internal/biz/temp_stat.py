...
class DwStat:
    ...
    @staticmethod
    def __new_case_summary1(data: list) -> Optional[str]:
        """
        立案信息总结
        :param json_record: mongo record
        :return: 总结
    ...
    async def __financing_summary(self, data: list):
        ...
        if re.search('[0-9]+', latest_amount):
            ...
            if len(str(int(latest_num))) >= 5:
                latest_num_str = str(latest_num / 10000) + '亿人民币'
                s3 = f'''最近一次融资金额：约{latest_num_str}，日期：{data[0]['Date']}\n'''
            else:
                latest_num_str = str(latest_num) + '万人民币'
                s3 = f'''最近一次融资金额：约{latest_num_str}，日期：{data[0]['Date']}\n'''
        else:
        ...
        if len(amount_3_num) > 0:
            if len(str(int(sum(amount_3_num)))) >= 5:
                amount_3_str = str(sum(amount_3_num) / 10000) + '亿人民币'
            else:
                amount_3_str = str(sum(amount_3_num)) + '万人民币'
        ...
        for i in range(len(investors)):
            ...
            cur = db["qcc_investors"].find({'Alias': name}, {})
            res = []
            async for docs in cur:
                res.append(docs)
            if len(res) > 0:
